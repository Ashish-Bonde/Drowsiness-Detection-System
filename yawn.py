...
# Load the data
train_img=train.flow_from_directory('dataset_new/Yawn/train',target_size=(150,150),batch_size=32,class_mode='binary') # Load the training data from the directory | target_size is the size of the image | batch_size is the number of images to be loaded at a time | class_mode is the type of classification (binary in this case)
test_img=test.flow_from_directory('dataset_new/Yawn/test',target_size=(150,150),batch_size=32,class_mode='binary') # Load the testing data from the directory | target_size is the size of the image | batch_size is the number of images to be loaded at a time | class_mode is the type of classification (binary in this case)

#Train and Test the model
eyes_model=mymodel.fit(train_img,epochs=35,validation_data=test_img) # Train the model with the training data | epochs is the number of times the model will be trained on the data | validation_data is the testing data

# Save the model
mymodel.save('yawn.h5') # Save the model to a file | 'eyes.h5' is the name of the file where the model will be saved